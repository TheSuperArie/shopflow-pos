...
export default function AdminLowStock() {
  ...
  lowStockVariants.forEach(variant => {
    ...
    if (group) {
      if (!groupedLowStock[group.id]) {
        groupedLowStock[group.id] = {
          group,
          variants: [],
          category: categories.find(c => c.id === group.category_id),
        };
      }
      groupedLowStock[group.id].variants.push(variant);
    }
  });

  const lowStockGroups = Object.values(groupedLowStock);
    ...
    <div className="space-y-6">
      ...
      {lowStockGroups.length === 0 ? (
        ...
        <div className="space-y-4">
          {lowStockGroups.map(({ group, variants, category }) => (
            <Card key={group.id} className="border-red-200 bg-red-50/30">
              <CardHeader className="pb-3">
                <div className="flex items-start justify-between">
                  ...
                  <Badge className="bg-red-600 text-white">
                    {variants.length} וריאציות חסרות
                  </Badge>
                </div>
              </CardHeader>
              <CardContent>
                      ...
                      return (
                        <div 
                          ...
                          className={`flex items-center justify-between p-3 rounded-lg ${
                            isCritical ? 'bg-red-100 border border-red-300' : 'bg-white border border-red-200'
                          }`}
                        >
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-xs text-gray-500">
                                מחיר: ₪{group.has_uniform_price ? group.uniform_sell_price : variant.sell_price}
                              </p>
                            </div>
                          </div>
