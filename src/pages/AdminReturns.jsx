...
export default function AdminReturns() {
  ...
  const approveReturnMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({ title: '✅ ההחזרה אושרה', description: 'המלאי עודכן והזיכוי נוצר', duration: 3000 });
    },
  });

  const rejectReturnMutation = useMutation({
    mutationFn: async (returnId) => {
      await base44.entities.Return.update(returnId, { status: 'נדחה', processed_by: 'admin' });
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['returns'] });
      toast({ title: 'ההחזרה נדחתה', duration: 2000 });
    },
  });
  ...
  const totalRefundAmount = returns.filter(r => r.status === 'אושר').reduce((sum, r) => sum + (r.total_amount || 0), 0);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        <Card>
          <CardHeader className="pb-3"><CardTitle className="text-sm font-medium text-gray-500">סה"כ החזרות</CardTitle></CardHeader>
          <CardContent><p className="text-3xl font-bold text-green-600">{approvedCount}</p></CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-3"><CardTitle className="text-sm font-medium text-gray-500">סכום החזרות</CardTitle></CardHeader>
          <CardContent><p className="text-3xl font-bold text-purple-600">₪{totalRefundAmount.toLocaleString()}</p></CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-3"><CardTitle className="text-sm font-medium text-gray-500">יתרת זיכויים</CardTitle></CardHeader>
          <CardContent><p className="text-3xl font-bold text-blue-600">₪{totalCreditBalance.toLocaleString()}</p></CardContent>
        </Card>
      </div>
      ...
      <div className="flex gap-2 flex-wrap">
        {['all', 'אושר', 'נדחה'].map(status => (
          <Button key={status} onClick={() => setFilterStatus(status)}
            variant={filterStatus === status ? 'default' : 'outline'} size="sm">
                ...
                <div className="flex items-start justify-between mb-3">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      {returnItem.refund_method && <Badge variant="outline">{returnItem.refund_method}</Badge>}
                    </div>
                  ...
                  <Button variant="outline" size="sm" onClick={() => setSelectedReturn(returnItem)}>פרטים</Button>
                </div>
              </CardContent>
            </Card>
          ))}
          {filteredReturns.length === 0 && <p className="text-center text-gray-400 py-12">אין החזרות להצגה</p>}
        </div>
      )}
